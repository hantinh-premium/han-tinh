...
/*
  DESIGN TOKENS — Tailwind Zinc + single emerald accent
  Page:    #09090B   Card:    #18181B   Elevated: #1F1F23
  Border:  #27272A   Hover:   #3F3F46
  Text:    #FAFAFA / #A1A1AA / #71717A / #52525B / #3F3F46
  Accent:  #10B981
*/
...
const FN="Inter,-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',sans-serif";
...
/* Tooltip */
function Tip({active,payload,label}){if(!active||!payload?.length)return null;return<div style={{background:"#27272A",border:"1px solid #3F3F46",borderRadius:8,padding:"8px 12px",fontSize:12,fontFamily:FN}}><div style={{color:"#71717A",fontSize:10,fontWeight:600,marginBottom:4,textTransform:"uppercase",letterSpacing:".05em"}}>{label}</div>{payload.map((p,i)=><div key={i} style={{color:"#FAFAFA",fontWeight:600}}>{p.name}: <span style={{color:p.color||A}}>{p.value}</span></div>)}</div>}
 
/* Badge — no border, squared, tiny */
function Bd({t,v}){const m={ok:["#10B98118",A],er:["#EF444415","#EF4444"],wa:["#F59E0B12","#F59E0B"],in:["#3B82F612","#3B82F6"],mu:["#27272A","#71717A"],pu:["#8B5CF612","#8B5CF6"],or:["#F9731612","#F97316"]};const[bg,fg]=m[v]||m.mu;return<span style={{display:"inline-block",padding:"2px 8px",borderRadius:4,fontSize:10,fontWeight:600,background:bg,color:fg,lineHeight:"18px"}}>{t}</span>}
...
useEffect(()=>{
  ...
  const lk=document.createElement("link");lk.id="_ht_font";lk.rel="stylesheet";lk.href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap";document.head.appendChild(lk);
},[]);
...
::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:#27272A;border-radius:99px}
input[type="date"]::-webkit-calendar-picker-indicator{filter:invert(.5)}
select{color-scheme:dark}::selection{background:#10B98130}
 
._c{background:#18181B;border:1px solid #27272A;border-radius:12px;padding:20px;transition:border-color .2s}
._c:hover{border-color:#3F3F46}
 
._ni{display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:500;color:#71717A;transition:all .15s}
._ni:hover{color:#A1A1AA;background:#1F1F23}
._ni._a{color:#FAFAFA;background:#27272A;font-weight:600}
 
._ba{display:inline-flex;align-items:center;gap:6px;padding:9px 18px;border-radius:8px;border:none;cursor:pointer;font-size:13px;font-weight:600;font-family:${FN};background:${A};color:#fff;transition:all .15s}
._ba:hover{filter:brightness(1.15)}
._bs{padding:5px 12px;font-size:11px;border-radius:6px}
._bg{padding:9px 18px;background:#27272A;border:none;border-radius:8px;font-size:13px;color:#A1A1AA;cursor:pointer;font-family:${FN};font-weight:500;transition:all .15s}
._bg:hover{background:#3F3F46;color:#FAFAFA}
._bo{display:inline-flex;align-items:center;gap:4px;padding:5px 12px;border-radius:6px;border:1px solid #27272A;background:transparent;cursor:pointer;font-size:11px;font-weight:600;font-family:${FN};color:#71717A;transition:all .15s}
._bo:hover{border-color:${A};color:${A}}
 
table{width:100%;border-collapse:collapse;min-width:600px}
th{padding:10px 16px;text-align:left;font-size:11px;font-weight:600;color:#3F3F46;text-transform:uppercase;letter-spacing:.05em;background:#111113;border-bottom:1px solid #27272A}
td{padding:11px 16px;font-size:14px;border-bottom:1px solid #1F1F23;color:#71717A;transition:background .1s}
tr:hover td{background:#1F1F23}
 
._ab{padding:5px;border-radius:6px;border:none;background:transparent;color:#3F3F46;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;transition:all .1s}
._ab:hover{color:${A};background:#18181B}._ab._d:hover{color:#EF4444;background:#18181B}
 
._tb{padding:6px 14px;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;color:#52525B;border:none;background:transparent;font-family:${FN};transition:all .15s}
._tb:hover{color:#A1A1AA}._tb._a{background:#27272A;color:#FAFAFA}
 
._in{width:100%;padding:10px 12px;border:1px solid #27272A;border-radius:8px;font-size:14px;outline:none;font-family:${FN};background:#09090B;color:#FAFAFA;margin-bottom:10px;transition:border-color .15s,box-shadow .15s}
._in:focus{border-color:${A};box-shadow:0 0 0 3px #10B98112}._in::placeholder{color:#3F3F46}
 
._pb{height:3px;background:#27272A;border-radius:99px;overflow:hidden}._pf{height:100%;border-radius:99px}
 
._bn{display:flex;background:#09090B;border-top:1px solid #27272A;padding:6px 0}
._bi{flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;padding:8px 0;cursor:pointer;color:#3F3F46;transition:.15s}._bi._a{color:${A}}
...
._mo{position:fixed;inset:0;background:rgba(0,0,0,.6);backdrop-filter:blur(20px);display:flex;align-items:center;justify-content:center;z-index:100;animation:_fi .1s}
._mp{background:#18181B;border:1px solid #27272A;border-radius:14px;padding:24px;width:500px;max-height:88vh;overflow-y:auto;animation:_si .15s ease-out}
._mm{width:100%;max-height:92vh;border-radius:14px 14px 0 0;position:fixed;bottom:0;left:0;right:0;animation:_su .2s ease-out;padding:20px 16px 28px}
._xb{background:#27272A;border:none;width:28px;height:28px;border-radius:6px;cursor:pointer;color:#71717A;display:flex;align-items:center;justify-content:center;transition:all .1s}._xb:hover{background:#3F3F46;color:#FAFAFA}
`}</style>
