...
const A = BRAND.pink;
const A2 = "#059669";
const FN = "'Be Vietnam Pro', Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
...
@media(max-width:980px){.flowline{grid-template-columns:1fr}.flow-arrow{height:18px;transform:rotate(90deg)}.analysis-grid,.decision-grid{grid-template-columns:1fr!important}.panel-title{font-size:12px}.hero-stat .big{letter-spacing:-.04em}.app{overflow:hidden}.shell{display:flex;height:100dvh}.main{height:100dvh;min-height:0}.content{height:calc(100dvh - 62px);overflow:auto}.sidebar{display:none}.topbar{height:62px;padding:0 14px}.content{padding:14px 14px 92px}.hero{padding:22px;border-radius:24px}.hero-inner{display:block}.hero-stat{text-align:left;margin-top:26px}.hero-actions{justify-content:flex-start;flex-wrap:wrap}.grid.kpis,.grid.two,.grid.three{grid-template-columns:1fr}.form-grid{grid-template-columns:1fr}.mobile-nav{display:flex;position:fixed;left:0;right:0;bottom:0;height:72px;padding-bottom:env(safe-area-inset-bottom);background:rgba(5,5,11,.92);border-top:1px solid rgba(255,255,255,.08);z-index:15;backdrop-filter:blur(24px)}.mobile-item{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;color:${BRAND.dim};font-size:9px;font-weight:950}.mobile-item.active{color:${A}}.page-title{font-size:34px}.panel,.kpi-card{border-radius:20px}.modal{border-radius:24px 24px 0 0;position:fixed;bottom:0;left:0;right:0;max-height:92vh}.search{width:100%}.search.desktop{display:none}.brand-mobile{display:flex!important}}`}</style>
  );
}
...
function Home({ isAdmin, students, leads, finance, trials, reports, hsk, setPg }) {
  ...
  return (
    <>
      ...
      {isAdmin && (
        <Panel title="Bản đồ vận hành hôm nay" className="ops-map">
          <div className="flowline">
            <div className="flow-step"><b>1</b><strong>Lead mới</strong><span>{leads.filter((l) => l.stage === "inquiry").length} cần tư vấn</span></div>
            <div className="flow-arrow">→</div>
            <div className="flow-step"><b>2</b><strong>Học thử</strong><span>{trials.filter((t) => t.status === "scheduled").length} lịch chờ</span></div>
            <div className="flow-arrow">→</div>
            <div className="flow-step"><b>3</b><strong>Đăng ký</strong><span>{leads.filter((l) => l.stage === "registered").length} đã chốt</span></div>
            <div className="flow-arrow">→</div>
            <div className="flow-step"><b>4</b><strong>Theo học</strong><span>{active.length} đang học</span></div>
            <div className="flow-arrow">→</div>
            <div className="flow-step danger"><b>5</b><strong>Học phí</strong><span>{overdue.length + pending.length} cần xử lý</span></div>
          </div>
        </Panel>
      )}
      ...
      {isAdmin && (
        <div className="grid two analysis-grid" style={{ marginBottom: 16 }}>
          ...
          <ChartBox title="Điểm số học viên">
            <BarChart data={[
              { r: "<5", n: students.filter((s) => (s.score || 0) < 5).length },
              { r: "5-6.5", n: students.filter((s) => (s.score || 0) >= 5 && s.score < 6.5).length },
              { r: "6.5-8", n: students.filter((s) => (s.score || 0) >= 6.5 && s.score < 8).length },
              { r: "8-9", n: students.filter((s) => (s.score || 0) >= 8 && s.score < 9).length },
              { r: "9+", n: students.filter((s) => (s.score || 0) >= 9).length },
            ]}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,.05)" />
              <XAxis dataKey="r" stroke="#6B6478" fontSize={11} tickLine={false} axisLine={false} />
              <YAxis stroke="#6B6478" fontSize={11} tickLine={false} axisLine={false} />
              <Tooltip content={<Tip />} />
              <Bar dataKey="n" name="Học viên" fill={BRAND.green} radius={[8, 8, 0, 0]} />
            </BarChart>
          </ChartBox>
      ...
      {isAdmin && (
        <div className="grid three decision-grid" style={{ marginBottom: 16 }}>
          <Panel title="Ưu tiên 1"><div className="decision-card"><strong>Chốt học thử gần nhất</strong><span>Ưu tiên các bạn đã học thử xong nhưng còn đang suy nghĩ. Đây là điểm chuyển đổi có xác suất cao nhất.</span></div></Panel>
          <Panel title="Ưu tiên 2"><div className="decision-card"><strong>Thu học phí quá hạn</strong><span>Khoản quá hạn làm đứt dòng tiền. Cần nhắc nhẹ, rõ hạn, có kịch bản gia hạn hoặc chia đợt.</span></div></Panel>
          <Panel title="Ưu tiên 3"><div className="decision-card"><strong>Giữ chuyên cần</strong><span>Học viên vắng nhiều cần được hỏi thăm sớm trước khi chuyển thành tạm nghỉ hoặc nghỉ học.</span></div></Panel>
        </div>
      )}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    lk.id = "_ht_font";
    lk.rel = "stylesheet";
    lk.href = "https://fonts.googleapis.com/css2?family=Be+Vietnam+Pro:wght@400;500;600;700;800;900&family=Inter:wght@400;500;600;700;800;900&display=swap";
